...
const fetchPersonalTutor = async () => {
  return await axios.get("http://localhost:5000/records/personalTutor").then((res) => res.data);
};

const GetPersonalTutor = () => {
  const [personalTutor, setPersonalTutor] = useState();
  useEffect(() => {
    fetchPersonalTutor().then((data) => setPersonalTutor(data.personalTutorRecord));
  }, []);
      ...
      <div>
        ...
        <div className="buttons">
          <Grid container spacing={2}>
            ...
            <Grid item xs={4}>
              <Link to={"/personalTutor/create"} style={{ textDecoration: "none" }}>
            ...
            <Grid item xs={4}>
              <Link to={"/personalTutor/search"} style={{ textDecoration: "none" }}>
                <Button sx={{marginTop : "19px"}}
                  variant="contained"
                  color="secondary"
        ...
        <div className="containList">
          {personalTutor &&
            personalTutor.map((personaltutorrecord, i) => (
              <div key={i}>
                <PersonalTutorRecord personaltutorrecord={personaltutorrecord} />
              </div>
            ))}
        </div>
      </div>
